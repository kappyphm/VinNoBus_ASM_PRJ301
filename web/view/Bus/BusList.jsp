...
<html>
    ...
    <body class="font-[Segoe_UI] bg-gradient-to-br from-[#eaf2f8] via-[#f5f7fa] to-[#e8f6f3] min-h-screen">
        <div
            class="w-[90%] mx-auto my-16 bg-white/90 backdrop-blur-sm p-10 rounded-2xl shadow-[0_12px_30px_rgba(0,0,0,0.08)] transition-all duration-300 animate-[fadeSlideUp_0.7s_ease]"
            >
            <!-- Tiêu đề -->
            <h2
                class="text-center text-3xl font-bold text-[#2c3e50] mb-8 relative after:content-[''] after:block after:w-32 after:h-1 after:rounded-full after:mx-auto after:mt-3 after:bg-gradient-to-r after:from-[#3498db] after:to-[#2ecc71] after:shadow-[0_2px_8px_rgba(52,152,219,0.4)]"
                >
                Danh sách Bus
            </h2>
                            ...
                            <tr class="even:bg-gray-50 odd:bg-white hover:bg-[#e8f4fd] transition duration-300">
                                ...
                                <td class="py-3 px-4 border border-gray-200 flex flex-wrap justify-center gap-2">
                                    <a
                                        ...
                                        class="bg-gradient-to-r from-[#f39c12] to-[#f1c40f] text-white px-3 py-1.5 rounded-md hover:opacity-90 transition"
                                        >✏️ Edit</a
                                    >
                                    <a
                                        href="BusServlet?action=delete&id=${bus.busId}"
                                        onclick="return confirm('Bạn có chắc muốn xóa?');"
                                        class="bg-gradient-to-r from-[#e74c3c] to-[#c0392b] text-white px-3 py-1.5 rounded-md hover:opacity-90 transition"
                                        >🗑️ Delete</a
                                    >
                                    <a
                ...
                <c:forEach var="i" begin="1" end="${totalPages}">
                    <c:choose>
                        <c:when test="${i == currentPage}">
                            <a
                                href="BusServlet?action=list&page=${i}&search=${param.search}&sort=${param.sort}"
