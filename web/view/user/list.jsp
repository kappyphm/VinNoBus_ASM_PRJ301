...
<main class="h-[100vh] max-w-6xl mx-auto px-5 py-10">
    ...
    <form action="${ctx}/users" method="get" class="flex flex-wrap gap-3 items-center mb-6">
        <!-- Loại user (ẩn nhưng quan trọng để servlet biết tab nào đang hiển thị) -->
        ...
        <button 
            type="submit" 
            class="px-3 py-2 bg-brand-600 text-white text-sm rounded-xl hover:bg-brand-700">
            Tìm kiếm
        </button>
...
<script>
    ...
    tabCustomer.addEventListener("click", () => {
        tabCustomer.classList.add("tab-active");
        tabCustomer.classList.remove("tab-inactive");
    ...
    tabStaff.addEventListener("click", () => {
        ...
        tabCustomer.classList.add("tab-inactive");
        tabCustomer.classList.remove("tab-active");
