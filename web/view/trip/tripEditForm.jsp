        ...
        <div class="max-w-2xl mx-auto bg-white/90 backdrop-blur-sm rounded-2xl shadow-md p-8 mt-10">
            ...
            <form action="TripServlet?action=update" method="post">
                        ...
                        <select id="routeId" name="routeId" required
                            ...
                            <c:forEach var="route" items="${routesList}">
                                <option value="${route.routeId}"
                                        <c:if test="${trip.routeId == route.routeId}">selected</c:if>>
                        ...
                        <select id="busId" name="busId"
                            ...
                            <c:forEach var="bus" items="${busesList}">
                                <option value="${bus.busId}"
                                        <c:if test="${trip.busId == bus.busId}">selected</c:if>>
                                    ${bus.plateNumber} (ID: ${bus.busId})
                                </option>
                            </c:forEach>
                        </select>
                    </div>
                    ...
                    <select id="driverId" name="driverId"
                        ...
                        <c:forEach var="driver" items="${driversList}">
                            <option value="${driver.userId}" 
                                    <c:if test="${trip.driverId == driver.userId}">selected</c:if>>
                                ${driver.name} (ID: ${driver.userId})
                            </option>
                        </c:forEach>
                    </select>
                </div>
                ...
                <div class="mb-5">
                    ...
                    <select id="conductorId" name="conductorId"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
                            focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5">
                        <option value="">-- Chọn phụ xe --</option>
                        <c:forEach var="conductor" items="${conductorsList}">
                            <option value="${conductor.userId}"
                                    <c:if test="${trip.conductorId == conductor.userId}">selected</c:if>>
                                ${conductor.name} (ID: ${conductor.userId})
                            </option>
                        </c:forEach>
                    </select>
                </div>
