...
<body class="bg-brand-50 text-slate-800 min-h-screen">
  <header class="border-b border-slate-200 bg-white">
    <div class="max-w-6xl mx-auto px-5 py-4 flex items-center justify-between">
      <a href="info.jsp" class="flex items-center gap-2">
      ...
      <div>
        ...
        <select name="a" required class="w-full rounded-xl border border-slate-200 px-3 py-2 bg-white">
          ...
          <c:forEach var="s" items="${stations}">
            <c:set var="sid" value="${fn:split(s,'|')[0]}" />
            <c:set var="sname" value="${fn:split(s,'|')[1]}" />
            <option value="${sid}" ${param.a == sid ? 'selected' : ''}>${sname}</option>
          </c:forEach>
        </select>
      </div>
      ...
      <div>
        ...
        <select name="b" required class="w-full rounded-xl border border-slate-200 px-3 py-2 bg-white">
          ...
          <c:forEach var="s" items="${stations}">
            <c:set var="sid" value="${fn:split(s,'|')[0]}" />
            <c:set var="sname" value="${fn:split(s,'|')[1]}" />
            <option value="${sid}" ${param.b == sid ? 'selected' : ''}>${sname}</option>
          </c:forEach>
        </select>
      </div>
              ...
              <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                  <c:forEach var="r" items="${routes}">
                    <tr>
                      <td class="py-2 pr-4 font-medium">${r.code}</td>
                      <td class="py-2 pr-4">${r.name}</td>
                      <td class="py-2 pr-4">${r.stationCount}</td>
